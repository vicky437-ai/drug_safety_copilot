...
SELECT AE_CLASSIFICATION:labels[0]::VARCHAR AS SOC_CLASS, COUNT(*) AS CNT
FROM D_BRONZE.CLASSIFIED_EVENTS
GROUP BY 1
ORDER BY CNT DESC;
...
SELECT
    ...
    SNOWFLAKE.CORTEX.SENTIMENT(n.NARRATIVE_TEXT) AS NARRATIVE_SENTIMENT
...
SELECT 
    CASE_ID,
    EXTRACTED:response:primary_drug::VARCHAR AS DRUG,
    EXTRACTED:response:adverse_event::VARCHAR AS AE,
    EXTRACTED:response:causality::VARCHAR AS CAUSALITY,
    NARRATIVE_SENTIMENT
